...
def get_roll_pitch(data):
    ...
    return roll,pitch
def get_movements(roll, pitch):
    movements = {'left': 0, 'right': 0, 'front': 0, 'back': 0}
    if roll<0:
...
def start_iio(device: iio.Device):
    ...
    movement=get_movements(roll,pitch)
...
def simulate_movement(queue: Queue):
    ...
    while not exit_event.is_set():
        if start_event.is_set():
            ...
            #increm movem left value
           # movements=queue.get()
            #movements["left"] += 1
            #print(f"running, left={movements['left']}")
            # put it back
            #queue.put(movements)
            #time.sleep(1)
        start_event.wait()
...
if __name__ == "__main__":
    ...
    print("Press Left Ctrl to start/stop aplic. Press Esc to exit")

    with keyboard.Listener(on_press=on_keypress) as listener:
        listener.join()
    

#aawwawawawaawwawwwawwwddwddsddssddwwdwwwwwwwwwwwwwdsdssdsddsssdsddssssssasssswwww
