...
def get_roll_pitch(data):
        ...
        roll = math.atan2(-x_accel, math.sqrt(y_accel ** 2 + z_accel)) * 180 / math.pi
...
def get_data(device: iio.Device):
        ...
        for i, channel_name in enumerate(channel_names):
                ...
                if i%2: axis_data[axes[i//2]]['-'] = int(attr)
                else: axis_data[axes[i//2]]['+'] = int(attr)
...
def get_movement(roll, pitch):
        ...
        if roll < 0: movements['right'] = min(1., abs(pitch) / limit)
        else: movements['left'] = min(1., abs(pitch) / limit)

        if pitch < 0: movements['back'] = min(1., abs(roll) / limit)
        else: movements['front'] = min(1., abs(roll) / limit)
...
def create_gui(queue: Queue):
        ...
        def update_sq_color(canvas, sq, value):
                canvas.itemconfig(sq, fill=f'#ffff{(255 - int(value * 255)):02x}')
